...
This parameter allows users to enable remote-write and an associated Thanos receiver instance for the full cluster-monitoring Prometheus TSDB blocks.

NOTE: This remote-write instance doesn't support customizing which metrics are sent to the receiver.
...
This parameter controls whether to deploy the Thanos receiver and associated remote-write config.
...
[source,yaml]
----
namespace: syn-openshift4-monitoring-thanos-receiver
----
example::
+
.Customize receiver local storage configuration
[source,yaml]
----
volumeClaimTemplate:
  spec:
...
This parameter allows users to customize the Thanos receiver configuration.
See https://github.com/thanos-io/kube-thanos/blob/main/jsonnet/kube-thanos/kube-thanos-receive-default-params.libsonnet[the upstream `kube-thanos` documentation] for supported parameters.

If field `namespace` isn't set to `openshift-monitoring`, the component will create the configured namespace.

The component provides a set of internal default configurations for the Thanos receiver.
...
ThanosReceiveHttpRequestLatencyHigh:
  patches:
    labels:
      severity: warning <3>
----
<1> add annotation `foo=bar` to all alerts
<2> disable alert `ThanosReceiveHttpRequestErrorRateHigh`
<3> lower the severity of alert `ThanosReceiveHttpRequestLatencyHigh` to `warning`

This parameter allows customizing the alerts for the remote write Thanos receiver.
Individual alert rules can be customized by adding an entry which uses the alert rule's name as the key.

The component understands fields `enabled` and `patches` in entry of the parameter.
Field `enabled` controls whether the alert should be deployed on the cluster.
Field `patches` can be used to specify custom configurations for the alert rule.
The value of field `patches` is merged as is with the specified alert rule.

The component supports a special key `*`, which can be used to provide configurations which are applied to all Thanos receiver alert rules.
If configuration for an individual alert is provided it takes precedence over configuration in `*`.

See the https://github.com/thanos-io/thanos/blob/main/mixin/alerts/receive.libsonnet[upstream Jsonnet] for the set of alerts which is deployed by default.
...
type: s3
config:
  bucket: ${cluster:name}-cluster-monitoring-thanos
  endpoint: s3.eu-central-1.example.com
  access_key: ?{vaultkv:${cluster:tenant}/${cluster:name}/cluster-monitoring/s3_access_key}
  secret_key: ?{vaultkv:${cluster:tenant}/${cluster:name}/cluster-monitoring/s3_secret_key}
----

This parameter allows users to configure the Thanos receiver's object storage configuration.
The contents of the parameter are passed verbatim to the receiver.

If the parameter value is the empty dictionary, the component assumes that the object storage configuration secret is managed externally.
In this case, the component expects that the user adjusts `receiver.objectStorageConfig` to point to the externally managed configuration secret.


See https://thanos.io/tip/thanos/storage.md/#configuring-access-to-object-storage[the upstream Thanos documentation] for supported configurations.
