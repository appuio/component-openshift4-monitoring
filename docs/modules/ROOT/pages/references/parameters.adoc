...
NOTE: The core logic of this feature is provided through Jsonnet library `alert-routing-discovery.libsonnet` that's provided through the https://github.com/projectsyn/jsonnet-libs.git[Project Syn shared Jsonnet libraries repository].
...
NOTE: Fields `ignoreNames` and `customAnnotations` of this parameter are structured to be suitable for Jsonnet library `alerts.libsonnet` that's provided through the https://github.com/projectsyn/jsonnet-libs.git[Project Syn shared Jsonnet libraries repository].
