...
This parameter allows users to configure the cluster-monitoring Prometheus Thanos sidecar to write TSDB blocks to object storage.
The contents of the parameter are passed verbatim to the Thanos sidecar as value for parameter `--objstore.config`.
If the parameter value is the empty dict (`{}`), the component won't deploy the Thanos sidecar configuration.
...
If field `type` is set to `s3`, the component will also create an ArgoCD sync job which creates the specified bucket, if it doesn't exist yet.

[NOTE]
====
This parameter uses Kyverno to patch the OpenShift-managed cluster-monitoring `Prometheus` custom resource.
Please make sure that Kyverno is enabled if you're using this feature.
====

[WARNING]
====
Modifying the cluster-monitoring `Prometheus` custom resource isn't officially supported by OpenShift, and may break after an OpenShift upgrade.
====
